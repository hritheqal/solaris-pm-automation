...
def get_row_value(row, possible_keys, default="N/A"):
...
def pdf_status_marks(status):
    ...
    if status_text == "ok":
        ok_mark = "X"
    elif status_text == "not ok":
        not_ok_mark = "X"
    elif status_text in ["not applicable", "n/a", "na"]:
        na_mark = "X"

    return ok_mark, not_ok_mark, na_mark
...
def get_finding_remark(findings, check_name):
    ...
    for finding in findings:
        ...
        if (
            ...
            return str(evidence)[:250]

    return ""


def make_pdf_checklist_table(title, rows, styles):
    elements = []
    ...
    table_data = [
        [
            ...
            Paragraph("PM Activity / Check Item", styles["TableHeader"]),
            Paragraph("OK", styles["TableHeader"]),
            Paragraph("Not OK", styles["TableHeader"]),
            Paragraph("N/A", styles["TableHeader"]),
    ...
    for index, item in enumerate(rows, start=1):
        ...
        ok_mark, not_ok_mark, na_mark = pdf_status_marks(status)

        if str(status).strip().lower() == "not captured":
            remarks = "Not captured from PM output"

        table_data.append(
            [
                str(index),
                Paragraph(pdf_safe(check_name), styles["TableCell"]),
                ok_mark,
                not_ok_mark,
                na_mark,
                Paragraph(pdf_safe(remarks), styles["TableCell"]),
            ]
        )

    table = Table(
        table_data,
        colWidths=[12 * mm, 70 * mm, 15 * mm, 18 * mm, 15 * mm, 50 * mm],
        repeatRows=1,
    )
...
def pdf_footer(canvas, doc):
    ...
    canvas.drawString(15 * mm, 10 * mm, "Generated by Solaris PM Automation")
    canvas.drawRightString(195 * mm, 10 * mm, f"Page {doc.page}")
    canvas.restoreState()


def generate_pdf_report(row, findings, pdf_path):
    ...
    elements.append(Paragraph("SOLARIS PM AUTOMATION", styles["ReportTitle"]))
    elements.append(Paragraph("PREVENTIVE MAINTENANCE REPORT", styles["ReportSubtitle"]))
    elements.append(Paragraph("Doc ID: PM-AUTO-001 | Version: 1.0", styles["ReportSubtitle"]))
    elements.append(Spacer(1, 8))

    overall_status = get_row_value(row, ["Overall Status", "Overall"])
    hostname = get_row_value(row, ["Hostname"])
    ...
    server_info = [
        ["Hostname", hostname, "Environment", "N/A"],
        ["Model", model, "Serial Number", serial],
        ["Firmware Version", firmware, "Location", "N/A"],
        ["OS Version", os_version, "Application", "N/A"],
        ["Date", date, "Uptime", uptime],
        ["Overall Status", overall_status, "PM Activity", "N/A"],
    ]

    server_table_data = []
    for row_item in server_info:
        server_table_data.append(
            [
                Paragraph(pdf_safe(row_item[0]), styles["TableHeader"]),
                Paragraph(pdf_safe(row_item[1]), styles["TableCell"]),
                Paragraph(pdf_safe(row_item[2]), styles["TableHeader"]),
                Paragraph(pdf_safe(row_item[3]), styles["TableCell"]),
            ]
        )
    ...
    elements.append(Paragraph("Server Information", styles["SectionTitle"]))
    elements.append(server_table)
    elements.append(Spacer(1, 10))

    physical_rows = [
        {
            "check_name": "PM Activity is Onsite / Offsite",
            "status": "Not Applicable",
            "remarks": "Not provided by Solaris PM output",
        },
        {
            "check_name": "Firmware Version Captured",
            "status": "OK" if firmware != "N/A" else "Not captured",
            "remarks": firmware,
        },
        {
            "check_name": "Fault LED / Hardware Diagnostic Status",
            "status": get_row_value(row, ["Hardware Diagnostic", "Hardware Diagnostic Status"]),
            "remarks": get_finding_remark(findings, "Hardware Diagnostic"),
        },
        {
    ...
    system_rows = [
        {
            "check_name": "OS Information",
            "status": "OK" if os_version != "N/A" else "Not captured",
            "remarks": os_version,
        },
        {
            "check_name": "OS Uptime",
            "status": "OK" if uptime != "N/A" else "Not captured",
            "remarks": uptime,
        },
        {
            "check_name": "Disk Management Status",
            "status": get_row_value(row, ["Disk Management", "Disk Management Status"]),
            "remarks": get_finding_remark(findings, "Disk Management"),
        },
        {
            "check_name": "Filesystem Status",
            "status": get_row_value(row, ["Filesystem", "Filesystem Status"]),
            "remarks": "Threshold fixed at 80%. " + get_finding_remark(findings, "Filesystem"),
        },
        {
            "check_name": "System Status from Syslog",
            "status": get_row_value(row, ["Syslog", "System Status from Syslog"]),
            "remarks": get_finding_remark(findings, "Syslog"),
        },
        {
            "check_name": "FMA Hardware Status",
            "status": get_row_value(row, ["FMA", "FMA Hardware Status"]),
            "remarks": get_finding_remark(findings, "FMA"),
        },
        {
            "check_name": "Services Status",
            "status": get_row_value(row, ["Services", "Services Status"]),
            "remarks": get_finding_remark(findings, "Services"),
        },
        {
            "check_name": "Hardware Diagnostic Status",
            "status": get_row_value(row, ["Hardware Diagnostic", "Hardware Diagnostic Status"]),
            "remarks": get_finding_remark(findings, "Hardware Diagnostic"),
        },
        {
            "check_name": "Hard Disk Device Statistics",
            "status": get_row_value(row, ["Hard Disk", "Hard Disk Device Statistics"]),
            "remarks": get_finding_remark(findings, "Hard Disk"),
        },
        {
            "check_name": "HBA Port Link Status",
            "status": get_row_value(row, ["HBA", "HBA Port Link Status"]),
            "remarks": get_finding_remark(findings, "HBA"),
        },
    ]
    ...
    virtualization_rows = [
        ...
        },
        {
            ...
            "status": get_row_value(row, ["VM Health", "VM Health Check"]),
            "remarks": get_finding_remark(findings, "VM Health"),
        },
    ]

    elements.extend(make_pdf_checklist_table("1. Physical Activities", physical_rows, styles))
    elements.extend(make_pdf_checklist_table("2. System Health Check", system_rows, styles))
    elements.extend(make_pdf_checklist_table("3. Virtualization", virtualization_rows, styles))

    elements.append(Paragraph("Overall Result", styles["SectionTitle"]))

    overall_table = Table(
        [
            [
                Paragraph("Overall PM Status", styles["TableHeader"]),
                Paragraph(pdf_safe(overall_status), styles["TableCell"]),
            ]
        ],
...
def process_file(input_file, output_dir="outputs"):
    ...
    generate_pdf_report(row, findings, pdf_path)
